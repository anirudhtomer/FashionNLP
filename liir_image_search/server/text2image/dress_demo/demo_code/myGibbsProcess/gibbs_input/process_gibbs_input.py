...
class LdaWriter(object):
    ...
    def write_2_lda(self, fout_name, item_list, item_type='freq', occurrence=False):
        """(str, lsg of lst, str) ->
        item_type can be either 'freq' or 'wordid'.  It refers to the type of items in
        the item list.
        An item list can be a frequency list, e.g., [[3, 2, 7], [2,8,9]]  means
        word0 occurs 3 times, word1 occurs 2 times, and so on
        Or an item list can be a wordid list, e.g., [[3, 2, 3, 2, 7, 5], [9,9,8,0]]
        means wordid 3 wordid 2 wordid3 and so on
        """
        f = open(fout_name, 'w')

        for doc in item_list:
            if item_type == 'freq':
                lda_string = self.freq_lst_2_lda_line(doc, occurrence)
            elif item_type == 'wordid':
                lda_string = self.wordid_lst_2_lda_line(doc)
            else:
                raise ValueError("item type must be 'freq' or 'wordid' ")
            f.write(lda_string + '\n')
        f.close()
        return

    def freq_lst_2_lda_line(self, freq_lst, occurrence=False):
        """(list) -> str
        input: freq_lst = [3, 2, 7]  means word0 occurs 3 times, word1 occurs 2 times, and so on
        output: '12 0:3 1:2 2:7' means 12 words and wordid:word_freqency
        """
        lda_line = ''
        ntokens = sum(x > 0 for x in freq_lst)  # count number of non-zero tokens

        lda_line += str(ntokens)

        idx = 0
        for actual_freq in freq_lst:
            freq = actual_freq
            if actual_freq == 0:
                idx += 1
                continue
            if occurrence:
                freq = 1
            lda_line += ' '
            lda_line += str(idx) + ":" + str(freq)
            idx += 1
        return lda_line

    def wordid_lst_2_lda_line(self, wordid_lst):
        """(list) -> str
        input: wordid_lst = [3, 2, 3, 2, 7, 5]  means wordid 3 wordid 2 wordid3 and so on
        output: '6 3:2 2:2 5:1 7:1' means 6 words and wordid:word_freqency
        """
        lda_line = ''
        nwords = len(wordid_lst)

        lda_line += str(nwords)

        counts = Counter(wordid_lst)

        for word_id in sorted(counts.keys()):
            freq = counts[word_id]
            lda_line += ' '
            lda_line += str(word_id) + ":" + str(freq)
...
class DocLine(object):
    ...
    def _parse_line(self):
        ...
        # print "wordid_list", self.wordid_list
        # print "\n"
